...
def generate_education(information: str):
    """Generate exam questions using Cohere"""
    student_info = f"""
    Use the following information: {information}.
    """
    try:
        response = co.generate(
            ...
            1. Generate exactly the number of exam questions requested.
            2. Each question must have the exact number of multiple-choice options requested.
            3. Mark the correct choice with "correct": true.
    ...
    if request.method == "POST":
        ...
        for key, value in request.POST.items():
            ...
            if key.startswith("question_"):
                ...
                total_questions += 1
                exam = question.education
                # assuming Choice.correct is a BooleanField
                if selected_choice.correct:
                    question.user_choice = selected_choice_id
                    question.is_correct = True
                    correct_answers += 1
                    exam.correct = int(exam.correct)+1
                else:
                    question.is_correct = False
                    exam.wrong = int(exam.correct)+1
                question.save()
                exam.save()
                # results_list.append({
                ...
                #     "correct": is_correct,
                #     "choices": [c.text for c in question.choices.all()]
                # })
        ...
        return render(request, "frontend/exam_results.html", context)

    # If not POST, redirect back to exam page
    messages.error(request, "You must submit the exam to see results.")
    return redirect("exam_page")  # replace with your exam page URL nam
            ...
            if form.is_valid():
                ...
                for q in questions:
                    question = Question.objects.create(
                        question=q["question"], user=user, education=found)
                    for choice in q["choices"]:
                        choice = Choice.objects.create(
                            question=question, choice=choice, user=user)
                        if choice.correct:
                            choice.correct = True
                        else:
